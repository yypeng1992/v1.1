...
#打开原文件/目标文件
##########################
my $orginal_file = "ddr_cfg_info.vh";   ##指定 配置源文件
my $result_file  = "ddr_cfg_info.coe";  ##指定 配置信息结果文件
...
while(<FIN>){
	...
	if($_ eq ''){
	    $log_line = $log_line+1;
		printf FOO "\@\@$log_line ----------- space line\n";
		next;
	};   ##忽略空白行
    chomp;
	...
	if($comment_flag==0){
		if(m{^/ \* }x){      ##注释头
			if(m{\* /$}x){   ##注释尾
				$comment_flag=0;
				$log_line = $log_line+1;
				printf FOO "\@\@$log_line --------------------multiline comment----------immediately done\n";
			}else{
				$comment_flag=1;
				$log_line = $log_line+1;
				printf FOO "\@\@$log_line --------------------multiline comment----------start-----\n";
			}
			next;
		}else{              ##无注释头
			...
			if($_ eq ''){
				$log_line = $log_line+1;
				printf FOO "\@\@$log_line --------------------singleline comment--\n";
				print  "$_\n";
				next;
			}else{
				$log_line = $log_line+1;
				printf FOO "\@\@$log_line --------valid ------singleline comment--\n";
				print  "$_\n";
				###################################################################
				...
				############ together cfg14 ############
					if(&ready(\%valid14_array)==1){
						...
						printf FOO "----------cfg15 info (default add)\n";
						printf FOUT "00000000000000000000000000000000\n\n";
						printf FOO "CFG15: 00000000000000000000000000000000\n\n";
						...
						if($all_cfg_info_done_flag == 1){
							printf FOUT ">>>>>>>>>>> END <<<<<<<<<<<\n";
							printf FOUT "Report: 你一次性配置了 $DDR_ACH_NUM 个地址通道!!\n";
							
							printf FOO ">>>>>>>>>>> END <<<<<<<<<<<\n";
							printf FOO "Stop at Line : $log_line\n";
							last;
						}
					}
				############ together cfg15 ############
			...
			if(m{\* /$}x){
				$comment_flag=0;
				$log_line = $log_line+1;
				printf FOO "\@\@$log_line --------------------multiline comment----------end-------\n";
			}else{
				$log_line = $log_line+1;
				printf FOO "\@\@$log_line --------------------multiline comment----------go on..---\n";
				next;
			}
	}
}
